...
try {
    $pdo = new PDO($dsn, $user, $pass,[
        ...
        PDO::ATTR_DEFAULT_FETCH_MODE => PDO::FETCH_ASSOC // Gallo 1: quité espacio
    ]);
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $apellido = trim($_POST["apellido"] ?? ""); // Gallo 3: quité espacios
    $correo = trim($_POST["correo"] ?? ""); // Gallo 3: quité espacios
    ...
    if ($nombre === "" || $apellido === "" || $correo === "") {
        ...
        try {
            ...
            $sqlAlumno = "INSERT INTO alumnos (nombre, apellido, correo)
                            VALUES (:nombre, :apellido, :correo)"; // Gallo 2: quité espacio en :correo
...
<head>
    ...
    input[type="text"], input[type="email"] {width:280px; padding: 8px; border: 1px solid #ccc; border-radius:6px} /* Gallo 4 */
    button{padding:10px 14px; border:0; border-radius:8px; cursor:pointer}
    .btn{background:#0b5ed7; color:white}
    .btn:hover{opacity:.9} /* Gallo 4 */
    ...
    <div class="card">
        <form method="POST">
            <div class="row">
                <div>
                    <label>Nombre</label>
                    <input type="text" name="nombre" maxlength="30" value="<?= htmlspecialchars($_POST['nombre'] ?? 'Jose Alfonso') ?>">
                </div>
                <div>
                    <label>Apellido</label>
                    <input type="text" name="apellido" maxlength="30" value="<?= htmlspecialchars($_POST['apellido'] ?? 'Aguilar') ?>">
                </div>
                <div>
                    <label>Correo</label>
